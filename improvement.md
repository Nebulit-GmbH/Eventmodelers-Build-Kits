...
**Reversed (2026-09-22)**: the singular/plural pairs were merged after all — the tool-schema cost was the deciding factor once it was measured (59 tools, ~40k characters of schema before a client sends anything). Eleven plural tools were deleted and their array parameter moved onto the singular name, which is the name every documented call site already used: `place_element.elements`, `set_connection.connections`, `create_screen.screens`, `add_comment.comments`, `create_drawing.drawings`, `delete_node.nodeIds`, `delete_column.columnIds`, `add_lane.lanes`, `create_slice_definition.slices`, `auto_connect_node.nodeIds`, `move_node_in_timeline.moves`. 59 tools -> 48, and the wire payload ~40.8k -> ~30.5k characters (25%).

The 2026-08-30 objection was right about the shape and was accepted rather than solved: **every merged tool now returns `{results: [...]}`, never the old flat object**, so a caller that read `response.nodeId` or `response.commentId` directly now reads `response.results[0].nodeId`. All Build-Kits call sites were migrated in the same change. The unresolved half of that objection still stands: this MCP server is exposed at `api.eventmodelers.ai`, and there is still no usage telemetry, so an external client on the flat shape breaks with no deprecation window. `create_screen` also gained batching for image and sketch content, which the old HTML-only `create_screens` never had.

**Objection closed (2026-09-22, same day)**: the flat-shape break above was not left standing. Every merged tool keeps its pre-merge flat params (`nodeId`, `elementType`/`title`, `source`/`target`/`action`, …) and answers them with the one result on its own, exactly as the singular tool did — including throwing on failure where it used to throw, rather than returning a success response with an `error` inside it. The array parameter always answers `{results: [...]}`, one entry or twenty.

**The response shape follows the parameter, never the entry count.** A first attempt keyed it off the number of entries, so a one-entry `nodeIds: ["n1"]` answered flat — which would have quietly falsified this repo's own skill text (`eventmodeling-orchestrating-event-modeling` promises that every entry reports its outcome in `results`) and forced batching callers to branch on cardinality. Keying it off the parameter also removed the `list` escape hatch that rule needed, so the merged tools carry one fewer param than the first attempt.

`oneOrMany` in `mcp/tools/helpers.ts` is a zod *preprocess*, not a union: a union emits the entry schema twice into the tool's JSON Schema, which would hand back the schema saving the merge was for. `backwardCompat.test.ts` pins the flat input and flat output per tool.

**What is still breaking, and is accepted**: the eleven plural tool *names* are gone, so a caller invoking `delete_nodes`/`place_elements`/… by name gets an unknown-tool error. No input coercion can cover that — only re-registering the names as deprecated aliases would, at roughly 1k characters of schema against the ~10k the merge saved. An LLM-driven client re-reads `tools/list` each session and is unaffected; a scripted client with hardcoded names is not.

**Placement tools folded in (2026-09-22)**: `drop_node_to_cell`, `move_node_in_timeline` and `clear_cell` were folded into `place_element` entries — all three were "node → cell on one timeline", and `dropNode` already moved a node that sat elsewhere, so drop and move were one operation. Each entry names its operation with `action` — `create` (default, so every pre-fold call is unchanged), `move` (`nodeId` + `cellName|cellId`: places or moves an existing node of any type; the type is read from the node, no `nodeType` param), `clear` (`cellName|cellId`: empties the cell, keeps the node). An explicit enum rather than inferring the operation from which keys are present, so a stray `nodeId` on a create entry fails loudly instead of silently becoming a move. The new modes take `cellName` ("B2", what `get_nodes` returns) as well as `cellId`, a first step toward one cell vocabulary. The three old tools are **kept, marked deprecated** in their descriptions (still 48 tools): removing them outright would break every agent following an installed kit from CLI ≤1.0.84, whose skills still call `drop_node_to_cell`. Remove them (48 -> 45) once a CLI release carrying the migrated skills has been out long enough. `move_timeline_structure` and `move_timeline_position` stay — they move columns/lanes and the chapter itself, not nodes.
