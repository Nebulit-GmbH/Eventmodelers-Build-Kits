...
program
  .name('build-kit-cratis-csharp')
  .description('build-kit-cratis-csharp — real-time Claude agent that turns Eventmodelers slices into Cratis (Arc + Chronicle) vertical slices')
  .version('0.0.1');
...
program
  ...
  .action(async () => {
    console.log('🚀 build-kit-cratis-csharp\n');

    const rootDir = process.cwd();
    const targetDir = join(rootDir, '.build-kit-cratis-csharp');
    ...
    for (const item of items) {
      ...
      // templates/build-kit/ contents spread into .build-kit-cratis-csharp/
      if (item === 'build-kit' && statSync(sourcePath).isDirectory()) {
        ...
        for (const kitItem of kitItems) {
          ...
          try {
            ...
            console.log(`  ✓ Installed .build-kit-cratis-csharp/${kitItem}`);
          } catch (err) {
      ...
      try {
        ...
        console.log(`  ✓ Installed .build-kit-cratis-csharp/${item}`);
      } catch (err) {
    ...
    // Add .build-kit-cratis-csharp/ to project root .gitignore
    const gitignorePath = join(rootDir, '.gitignore');
    const gitignoreEntry = '.build-kit-cratis-csharp/';
    ...
    console.log('       node .build-kit-cratis-csharp/ralph-claude.js\n');
    console.log('  Local Ollama model (run `ollama serve` first):');
    console.log('       OLLAMA_MODEL=qwen3:8b node .build-kit-cratis-csharp/ralph-ollama.js\n');
    console.log('  Pass a custom project directory as the first argument:');
    console.log('       node .build-kit-cratis-csharp/ralph-claude.js /path/to/project\n');
    console.log('Skills are ready in .build-kit-cratis-csharp/.claude/skills/ — use /connect to set a board ID.');
  });

program
  .command('uninstall')
  .description('Remove build-kit-cratis-csharp files from current directory')
  .action(() => {
    const targets = [
      join(process.cwd(), '.build-kit-cratis-csharp'),
    ];
...
program
  ...
  .action(() => {
    const kitDir = join(process.cwd(), '.build-kit-cratis-csharp');
    const skillsDir = join(kitDir, '.claude', 'skills');
    const configPath = join(kitDir, '.eventmodelers', 'config.json');

    console.log('.build-kit-cratis-csharp Status\n');
